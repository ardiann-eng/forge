...
import { forgeFactory, explorer } from '@/lib/config';
...
export function TokenTerminal({ address: tokenAddress }: { address: Address }) {
  ...
  const routerQuery = useQuery({
    queryKey: ['token-router', tokenAddress, address], enabled: !!forgeFactory,
    queryFn: async () => {
      const routerAddress = await publicClient.readContract({ address: requireFactory(), abi: factoryAbi, functionName: 'tokenToRouter', args: [tokenAddress] });
  ...
  const candlesQuery = useQuery({
    ...
    queryFn: async () => { const res = await fetch(`/api/market/candles?token=${tokenAddress}&timeframe=${timeframe}&metric=${metric}`); if (!res.ok) throw new Error('Failed to load candles'); return res.json() as Promise<{ candles: Candle[]; events: ForgeChartEvent[]; activity: LiveActivityItem[]; trades: LiveTrade[]; source: string; updatedAt: number }>; },
    refetchInterval: 10_000,
  });
