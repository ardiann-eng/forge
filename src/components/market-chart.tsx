...
export function MarketChart({ candles, events, timeframe, onTimeframeChange, metric, onMetricChange, tokenSymbol, currentPriceUsd, currentMarketCapUsd, priceChange24h, isLive = true, isLoading = false, isError = false, canShowMarketCap = false }: MarketChartProps) {
  ...
  return <div className="modern-terminal-chart-card">
    ...
    {events.length ? <div className="chart-event-filter-bar"><span className="event-filter-label"><Zap size={13} />FORGE EVENTS</span><div className="event-filter-pills">{['ALL', 'BUYBACK', 'BURN', 'LIQUIDITY', 'HOLDER_REWARD'].map((tag) => <button key={tag} type="button" className={`event-pill-btn ${activeEventFilter === tag ? 'is-active' : ''}`} onClick={() => setActiveEventFilter(tag)}>{tag === 'HOLDER_REWARD' ? 'HOLDERS' : tag}</button>)}</div></div> : null}
